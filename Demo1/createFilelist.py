...
def writepath(folderpath, listpath, root,file_path):
    ...
    for list in listpath:
        ...
        if os.path.isdir(path) and len(os.listdir(path)) > 0:
            writepath(tem, os.listdir(tem), root + '\\' +list, file_path)
            # file_path.write(list)
            # file_path.write("\\")
            # file_path.write("|")
            # file_path.write(mtime)
            # file_path.write("\n")
